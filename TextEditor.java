	...
	public static void main(String args[]) throws IOException {
		...
		while(!command.contains("EXIT"))
		{
			...
			else if(command.contains("WRTE"))
			{
				...
				if(responseFromServerWrite.contains(response[0]) && !responseFromServerWrite.contains(response[1]))
				{
					int versionCurrent = myObje.getVersionOfClient();
					versionCurrent++;
					myObje.setVersionOfClient(versionCurrent);
			...
			else if(command.contains("APND"))
			{
				...
				System.out.println(responseFromServerapn);
				if(responseFromServerapn.contains(response[0]) && !responseFromServerapn.contains(response[1]))
				{
					int versionCurrent = myObje.getVersionOfClient();
					versionCurrent++;
					myObje.setVersionOfClient(versionCurrent);
